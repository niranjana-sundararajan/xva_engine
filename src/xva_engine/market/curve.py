...
class DiscountCurve:
    ...
    def __init__(self, curve_data: CurveSnapshot):
        ...
        self.dfs = np.array(
            [p.discount_factor for p in points_sorted], dtype=np.float64
        )
    ...
    def df(self, t: float | np.ndarray) -> Union[float, np.ndarray]:
        ...
        first_slope = (self.log_dfs[1] - self.log_dfs[0]) / (
        ...
        if np.any(left_mask):
            log_df[left_mask] = self.log_dfs[0] + first_slope * (
                t_arr[left_mask] - self.days[0]
            )

        right_mask = t_arr > self.days[-1]
        if np.any(right_mask):
            log_df[right_mask] = self.log_dfs[-1]
    ...
    def zero_rate(self, t: float | np.ndarray) -> Union[float, np.ndarray]:
        """Continuously-compounded zero rate r(0,t) such that df(t) = exp(-r*t/365)."""
        t_years = np.maximum(np.asarray(t, dtype=np.float64) / 365.0, 1e-6)
        result = -np.log(self.df(t)) / t_years
        return result
