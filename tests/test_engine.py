...
def engine(mixed_netting_set, sample_curve_snapshot, sample_cpty_credit,
           sample_bank_credit, model_config, tmp_path_factory):
...
class TestEngineResultTypes:
    ...
    def test_V_sample_by_type_keys_match_ee_keys(self, result):
        assert set(result["V_sample_by_type"].keys()) == set(result["ee_by_type"].keys())
...
class TestEngineDeterminism:
    def test_same_seed_gives_same_CVA(
        self, mixed_netting_set, sample_curve_snapshot,
        sample_cpty_credit, sample_bank_credit, model_config, tmp_path_factory
    ):
...
class TestEngineSameTypeTrades:
    ...
    def test_two_zcbs_produce_valid_ee_by_type(
        self, sample_curve_snapshot, sample_cpty_credit,
        sample_bank_credit, model_config, tmp_path_factory
    ):
        ns = NettingSet(
            ...
            trades=[
                ZCBTa(trade_id="ZCB-A", netting_set_id="NS-2ZCB",
                      notional=500_000, maturity_date=365),
                ZCBTa(trade_id="ZCB-B", netting_set_id="NS-2ZCB",
                      notional=500_000, maturity_date=730),
            ],
        )
